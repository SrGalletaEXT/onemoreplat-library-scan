...
- On your own `steamcommunity.com` profile page: shows a small panel with the result of the last scan, plus a **"Buscar juegos"** button. Clicking it opens the store front page in a new tab (a real HTML page, not `dynamicstore/userdata`'s raw JSON directly -- some browsers won't run a userscript reliably on that) which runs the read-and-send step above and closes itself when done; the panel updates live the moment that happens. Blocked pop-ups and no-response cases are both detected and shown, so the button never gets stuck silently. It checks that the profile being viewed is **your own** before showing anything -- it never appears on someone else's profile, and viewing someone else's profile never sends their data anywhere.
